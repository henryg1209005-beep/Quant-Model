...
class TestFeatureInventory(unittest.TestCase):
    def test_inventory_reports_all_current_feature_groups(self) -> None:
        ...
        try:
            ...
            failed_break = next(item for item in report["items"] if item["feature"] == "failed_break")
            self.assertEqual(failed_break["present_count"], 1)
            self.assertIn("none", failed_break["examples"])
        finally:
